...
export function CoverImageCropModal({ file, onSave, onClose }: CoverImageCropModalProps) {
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      // Auto-fit: scale so the image covers the crop rectangle
      const container = containerRef.current;
      if (!container) return;
      const viewW = container.clientWidth;
      const viewH = container.clientHeight;
      const cropW = viewW * 0.9;
  ...
  // Get crop rectangle dimensions relative to canvas
  ...
  const draw = useCallback(() => {
    ...
    // Dim outside crop rectangle
    ctx.save();
    ctx.beginPath();
    ctx.rect(0, 0, viewW, viewH);
    // Cut out the crop rectangle (counter-clockwise for hole)
  ...
  async function handleSave() {
    ...
    const scaleX = OUTPUT_WIDTH / crop.w;
    const scaleY = OUTPUT_HEIGHT / crop.h;
    const drawW = img.width * zoom * scaleX;
    const drawH = img.height * zoom * scaleY;
    const drawX = (cx - img.width * zoom / 2 + offset.x - crop.x) * scaleX;
    const drawY = (cy - img.height * zoom / 2 + offset.y - crop.y) * scaleY;

    octx.drawImage(img, drawX, drawY, drawW, drawH);

    // Try WebP first, fall back to JPEG (Safari)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      <div
        className="bg-[var(--color-surface)] rounded-[var(--radius-organic)] shadow-xl w-full max-w-2xl mx-4 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--color-border)]">
          <h2 className="font-display text-lg text-[var(--color-ink)]">Edit cover image</h2>
          <button
            onClick={onClose}
        ...
        {/* Canvas area — wider aspect for cover */}
        <div
          ref={containerRef}
