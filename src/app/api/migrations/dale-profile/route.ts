...
export async function GET(request: NextRequest) {
  ...
  // List all operators so we can identify the correct slug
  const { data: allOperators, error: listError } = await client
    .from('operators')
    .select('id, slug, name, tagline')
    .order('created_at', { ascending: false });

  if (listError) {
    return NextResponse.json({ error: 'Could not list operators.', detail: listError.message }, { status: 500 });
  }

  // Allow override via ?slug= param, fall back to mallacoota-cruises
  const targetSlug = request.nextUrl.searchParams.get('slug') || 'mallacoota-cruises';

  const operator = allOperators?.find(op =>
    op.slug === targetSlug ||
    op.name?.toLowerCase().includes('mallacoota') ||
    op.name?.toLowerCase().includes('dale')
  );

  if (!operator) {
    return NextResponse.json({
      error: 'Operator not found. Check available operators.',
      availableOperators: allOperators?.map(op => ({ id: op.id, slug: op.slug, name: op.name })),
    }, { status: 404 });
  }
...
Group sizes are kept small by design. Every participant gets time at the rail. Dale's approach is methodical rather than theatrical — the list builds through careful scanning, not luck.`;
  ...
  const currentMetadata = (operator as { metadata?: Record<string, unknown> }).metadata ?? {};
  const newMetadata = {
    ...currentMetadata,
